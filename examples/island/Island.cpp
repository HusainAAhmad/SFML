...
// When running on an X11 system we need to inform Xlib that
// we are going to be accessing it from multiple threads
...
int main()
{
    // Prepare Xlib for multi-threaded access on X11 systems
#if (defined(SFML_SYSTEM_LINUX) || defined(SFML_SYSTEM_FREEBSD))
